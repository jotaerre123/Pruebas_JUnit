...
class DemoApplicationTests {
	...
	void whenNueva_success(){
		...
		Venta venta = Venta.builder()
				.id(1L)
				.cliente(c)
				.lineasDeVenta(List.of(new LineaDeVenta( p, 3, 12.34)))
				.build();


		when(productoRepositorio.findById(any(Long.class))).thenReturn(java.util.Optional.ofNullable(p));

		when(ventaRepositorio.save(any(Venta.class))).thenReturn(venta);
